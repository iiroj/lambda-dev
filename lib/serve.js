...
exports.listen = async ({ basePath, entry, node: nodeVersion, port, webpackConfig: customConfig }) => {
  const app = await createServer({ basePath, entry, node: nodeVersion, port, webpackConfig: customConfig });
  const path = getPath(basePath);
