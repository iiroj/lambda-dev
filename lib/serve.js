...
const createHandler = (entry, nodeVersion) => (req, res) => {
  let lambda;

  try {
    lambda = requireFromString(transform(entry, nodeVersion), entry);
  } catch (err) {
...
const createDevServer = ({ basePath, entry, node: nodeVersion, port }) => {
  const path = basePath.endsWith('/') ? basePath.replace(/\/$/, '') : basePath;
  ...
  for (const lambda of functions) {
    app.all(path + lambda.pathname, createHandler(lambda.module, nodeVersion));
  }
