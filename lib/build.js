...
const getCallback = (entries, resolve, reject, callback) => (err, stats) => {
  if (err) {
    console.error(
      `${chalk.grey("λ-dev")} ${chalk.red("Webpack error:")} %s`,
      err
    );
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(
        `${chalk.grey("λ-dev")} ${chalk.red("Compilation error:")} %s`,
        error
      );
    }
  ...
  if (warnings.length > 0) {
    for (const warning of warnings) {
      console.warn(
        `${chalk.grey("λ-dev")} ${chalk.red("Compilation warning:")} %s`,
        err
      );
    }
  }
...
module.exports = ({
  ...
  new Promise((resolve, reject) => {
    ...
    try {
      ...
      console.error(
        `${chalk.grey("λ-dev")} ${chalk.red("Build error:")} %s`,
        err
      );
      reject({ entries, error: err });
    }
